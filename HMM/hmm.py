...
class HMM:
    ...
    def __init__(self,N,M):
        ...
        self.A = np.random.exponential(1,[N,N])
        self.B = np.random.exponential(1,[N,M])
    ...
    def loaddata(self,O):
        ...
        self.O = O
        self.Z = np.array([self.B[:,self.O[t]] for t in range(len(self.O))])
    
    # Forward procedure (Eq. 19-21)
    def forward(self):
        
        # Initialization
        self.alpha = np.empty([len(self.O),len(self.Pi)])
        self.alpha[:] = np.NaN
        self.alpha[0] = self.Pi*self.Z[0]
        
        # Induction
        for t in range(len(self.O)-1):
            self.alpha[t+1] = np.dot(self.alpha[t],self.A)*self.Z[t+1]
    ...
    def backward(self):
        ...
        # Initialization
        self.beta = np.empty([len(self.O),len(self.Pi)])
        self.beta[:] = np.NaN
        self.beta[-1] = 1.0
        
        # Induction
        for t in range(len(self.O)-2,-1,-1):
            self.beta[t] = np.dot(self.beta[t+1]*self.Z[t+1],self.A.T)
